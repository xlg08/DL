...
def dm01():
    ...
    x = torch.tensor([10, 20], requires_grad=True, dtype=torch.float32)     # x 是自定义的权重
    ...
    # 真正写法.
    y.sum().backward()          # 本质上是 loss.sum().backward()
    # print("backward() 方法返回：", y.sum().backward())  # backward() 方法无返回值
    # print("y.sum()：", y.sum())      # tensor(1000., grad_fn=<SumBackward0>)
